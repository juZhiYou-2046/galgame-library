...
def upload_cover(game_id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if ext not in settings.allowed_extensions:
        raise HTTPException(
            status_code=400,
            detail=f"不支持的文件格式: {ext}，仅支持 {', '.join(sorted(settings.allowed_extensions))}",
        )

    # 生成唯一文件名，避免覆盖
    unique_name = f"{uuid.uuid4().hex}{ext}"
    save_path = settings.covers_path / unique_name
    ...
    # 删除旧封面文件
    if game.cover:
        old_path = settings.covers_path / Path(game.cover).name
        if old_path.exists():
            try:
                old_path.unlink()
            except OSError:
    ...
    # 更新数据库中的封面路径
    cover_url = f"{settings.COVERS_URL}/{unique_name}"
